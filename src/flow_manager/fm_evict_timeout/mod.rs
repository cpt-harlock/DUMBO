...
pub struct FlowManagerEvictTimeout {
    inner_struct: FlowManagerInnerStruct,
    timeout: f64,
}

impl FlowManagerEvictTimeout {
    pub fn build(rows: usize, slots: usize, packet_limit: usize, seed: usize, timeout: f64) -> FlowManagerEvictTimeout {
        FlowManagerEvictTimeout {
            inner_struct: FlowManagerInnerStruct {
...
impl FlowManager for FlowManagerEvictTimeout {
    ...
    fn set_evicted_next_block(&mut self, next_block: Rc<RefCell<dyn PacketReceiver>>) {
        self.get_fm_inner_struct().evicted_receiver = Some(next_block);
    }

    fn set_model_next_block(&mut self, next_block: Rc<RefCell<dyn PacketReceiver>>) {
        self.get_fm_inner_struct().model_receiver = Some(next_block);
    }

    fn get_received_flows_while_in_cache_collected(&mut self) -> Vec<(f64, String, usize, usize, usize)> {
        self.get_fm_inner_struct().collected_cache_stats_vector.to_vec()
    }

    fn get_received_flows_while_in_cache_evicted(&mut self) -> Vec<(f64, String, usize, usize, usize)> {
        self.get_fm_inner_struct().evicted_cache_stats_vector.to_vec()
    }

    fn clear(&mut self) {
        self.get_fm_inner_struct().hash_table.clear();
        self.get_fm_inner_struct().evicted_cache_stats_vector.clear();
        self.get_fm_inner_struct().collected_cache_stats_vector.clear();
        self.get_fm_inner_struct().fake_syn_set.clear();
    }
}

impl PacketReceiver for FlowManagerEvictTimeout {
    ...
    fn receive_packet<'a>(&'a mut self, packet: &'a mut Packet) -> Option<Rc<RefCell<dyn PacketReceiver>>> {
        ...
        if let Some(bucket) = inner_struct.hash_table.get_mut(&index) {
            ...
            if !hit {
                // if the flow is not already inside the Flow Manager and we alreay meet it
                ...
                if FAKE_SYN && inner_struct.fake_syn_set.contains(&flow_key) {
                    ...
                    if bucket.len() > inner_struct.slots {
                        ...
                        // in case we used the inference cache mechanism, we must avoid sending a
                        // packet
