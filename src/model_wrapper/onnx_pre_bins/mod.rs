...
pub struct ModelWrapperOnnxPreBins {
...
impl ModelWrapperOnnxPreBins {
    pub fn build(pickle_file: String, threshold: f64, features_file: String, bins_file: String) -> ModelWrapperOnnxPreBins {
        ...
        let mut ret = ModelWrapperOnnxPreBins { inner_struct, debug_inference_dump: HashMap::new(), features_file: features_file.clone(), bins_file: bins_file.clone() };
        ...
        // import bins data
        gil.python().run(&format!("bins_file = open(\"{}\", \"rb\")\n", bins_file), None, None).unwrap();
        gil.python().run(&format!("bins_edges = pickle.load(bins_file)\n"), None, None).unwrap();
        ret
    }
...
impl ModelWrapper for ModelWrapperOnnxPreBins {
...
impl PacketReceiver for ModelWrapperOnnxPreBins {
    fn receive_packet<'a>(&'a mut self, mut packet: &'a mut Packet) -> Option<Rc<RefCell<dyn PacketReceiver>>> {
        ...
        gil.python().run("X_onnx = np.concatenate([bins_edges.transform(X_onnx[bins_edges.feature_names_in_]), X_onnx[[f for f in used_features_vector if f not in bins_edges.feature_names_in_]].values], axis=1)\n", None, None).unwrap();
        gil.python().run("inputs = {'X': X_onnx}\n", None, None).unwrap();
